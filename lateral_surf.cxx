...
int main ( int argc, char* argv[] ) {
    ...
    TH2D* lateral_surface = new TH2D("top", "top surface",dr,r_min,r_max,dh,-h_max,-h_min);     //LA POSIZIONE FISICA DEL RIVELATORE NEL PLOT (VEDERE IL FILL DI TH2D)
...
//---- LETTURA DEI FILE ALLA XPOS E RAD CORRISPONDENTE AL CICLO -----------------------------------------------------------------------------
            ...
            for (string data_name : files) {
                ...
                size_t posr = data_name.find(r_str);                                        //gli dico di partire da 20 (Ex: x=3050---r=050)
                                                                                            //ambiguità con le x
...
//---- ANALISI DATI -------------------------------------------------------------------------------------------------------------------------
		...
		filename += data_name;
                ...
                lateral_surface -> Fill(r,-h,segnale);                                                         //riempio l'istogramma 2D
                //PER IL -H VEDERE LE PRIME RIGHE DOVE INIZIALIZZO IL TH2D
...
//-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    cout<<"file 'lateral_signal.txt' creato!\n\n";
    
    string h2d_name = PATH_OUT + "/histo2D_lateral.root";
    TFile* out_h2d = new TFile(h2d_name.c_str(), "RECREATE");                                         //crea il file root con TH2D-
    TCanvas* c = new TCanvas();

    lateral_surface -> GetXaxis()->SetTitle("Angle [Degree]");
    lateral_surface -> GetYaxis()->SetTitle("Position [0.1*mm]");
    lateral_surface -> GetZaxis()->SetTitle("Signal");
    lateral_surface -> Draw("colz");

    c -> Write();
    out_h2d -> Close();
    
    cout<<"File 'histo1D_lateral.root' e 'histo2D_lateral.root'  creati!"<<endl;
